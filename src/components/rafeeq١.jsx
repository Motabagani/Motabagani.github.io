...
.rafeeq .rq-return{
  ...
  background:linear-gradient(135deg, rgba(92,58,158,.55), rgba(32,16,61,.5));
  ...
  box-shadow: inset 0 1px 0 rgba(255,255,255,.30), 0 -12px 40px -16px rgba(10,4,24,.7);
  transition:background .18s ease;
}
.rafeeq[dir="rtl"] .rq-return{ font-family:"Mada", sans-serif; }
.rafeeq .rq-return:hover{ background:linear-gradient(135deg, rgba(110,70,180,.62), rgba(40,20,74,.55)); }
...
.rq-topbar{position:sticky;top:0;z-index:40;display:flex;align-items:center;gap:14px;justify-content:space-between;
  padding:12px clamp(16px,4vw,36px);background:var(--glass);backdrop-filter:blur(14px);
  border-block-end:1px solid var(--coral)}
.rq-topstart{display:flex;align-items:center;gap:14px;min-width:0}
...
/* chevron step bar — no transforms (Safari-safe). Order = natural RTL flex; arrows = clip-path */
.rq-chevs{display:flex;gap:6px;margin-block-end:18px}
.rq-chev{flex:1 1 auto;min-width:0;padding:11px 8px 11px 18px;background:var(--surface);border:1px solid var(--line);
...
.rq-chev:first-child{clip-path:polygon(0 0, calc(100% - 13px) 0, 100% 50%, calc(100% - 13px) 100%, 0 100%);padding-inline-start:12px}

/* RTL: natural flex already puts ١ on the right — do NOT reverse. Just mirror the point to the left. */
.rafeeq[dir="rtl"] .rq-chev{clip-path:polygon(100% 0, 13px 0, 0 50%, 13px 100%, 100% 100%, calc(100% - 13px) 50%);padding:11px 18px 11px 8px}
.rafeeq[dir="rtl"] .rq-chev:first-child{clip-path:polygon(100% 0, 13px 0, 0 50%, 13px 100%, 100% 100%);padding-inline-start:12px}
...
export default function RafeeqApp() {
  ...
  <div className="rq-logo">
    <div className="wm"><span className="wm-ar">رفيــق</span><span className="wm-en">Rafeeq</span></div>
    <Mark size={38} />
  </div>
  <div className="rq-topstart">
