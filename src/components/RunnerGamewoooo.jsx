// ============================================================
// RUNNER GAME 2.0
// Minimal aesthetic: driller on a horizon line, shrine obstacles,
// floating logo collectibles. Pre-start shows "Great stuff take time".
// ============================================================
...
// Shrine is wider than tall, so use separate dimensions
const OBSTACLE_WIDTH = 130;
const OBSTACLE_HEIGHT = 65;

const COLLECTIBLE_SIZE = 40;

const OBSTACLE_MIN_GAP = 1900;
const OBSTACLE_MAX_GAP = 2800;
...
function RunnerGame2() {
  ...
  if (playerYRef.current <= 0.1) {
    // Scale jump velocity with current speed too, so the arc shape stays consistent
    const speedScale = speedRef.current / BASE_SPEED;
    playerVelocityRef.current = JUMP_VELOCITY * Math.sqrt(speedScale);
  }
};
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      // Player physics
      // Player physics — scale gravity DOWN as speed increases, so the jump arc 
// covers more horizontal distance at higher speeds
const speedScale = speedRef.current / BASE_SPEED;     // 1.0 at start, grows over time
const effectiveGravity = GRAVITY / speedScale;        // weaker gravity = more hangtime
playerVelocityRef.current -= effectiveGravity * dt;
